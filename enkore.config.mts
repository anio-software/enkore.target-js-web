	...
	autogeneratedFiles: [
		defineAutogeneratedFile({
			...
			generator() {
				...
				code += `const api = await generateTargetIntegrationAPI("js-web");\n`
		...
		defineAutogeneratedFile({
			...
			generator() {
				return `export * from "@anio-software/enkore-private.target-js-factory/project"\n`
			}
		}),
		...
		defineAutogeneratedFile({
			...
			generator() {
				...
				code += `const api = await generateAutogenerateAPI("js-web");\n`
